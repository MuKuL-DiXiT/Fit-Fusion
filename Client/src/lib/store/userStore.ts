...
import { authService, type User as AuthUser, type SignupData, type LoginData } from '@/lib/auth/authService';
  ...
  persist(
    (set, get) => ({
      ...
      addToCart: (item: CartItem) => {
        ...
        if (existingItem) {
          get().updateCartQuantity(item.id, existingItem.quantity + item.quantity);
        } else {
