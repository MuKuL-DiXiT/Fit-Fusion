      ...
      if (items && items.length > 0) {
        for (const item of items) {
          await client.query(
            ...
            [planId, item.food_id || null, item.product_id || null, item.meal_time, item.quantity, item.calories]
          );
        }
      }
...
router.post('/:planId/items', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [planId, food_id || null, product_id || null, meal_time, quantity, calories]
    );
